...
from env.train_stage_b import (
    ExponentialMovingAverage,
    _training_loader_options,
    train_sfpd,
    train_sfps,
)
...
def test_cuda_training_loader_prefetches_like_the_reference_pusht_pipeline():
    assert _training_loader_options(torch.device("cuda:1")) == {
        "num_workers": 1,
        "pin_memory": True,
...
def test_cpu_training_loader_stays_single_process_without_pinned_memory():
    assert _training_loader_options(torch.device("cpu")) == {
        "num_workers": 0,
        "pin_memory": False,
...
def test_short_cpu_training_writes_finite_best_ema_checkpoint(tmp_path):
    ...
    assert loaded["metadata"]["trajectory_targets"] == {
        "training": "torch_uniform_first_order_hold_float32",
        "validation": "drake_first_order_hold_float64_boundary_float32_output",
    }
...
def test_short_sfps_training_is_finite_reproducible_and_restores_rng(tmp_path):
    ...
    assert one["metadata"]["trajectory_targets"] == {
        "training": "torch_uniform_first_order_hold_float32",
        "validation": "drake_first_order_hold_float64_boundary_float32_output",
    }
    for state_name in ("raw_state", "ema_state"):
...
def test_sfps_training_keeps_samplewise_drake_out_of_the_training_hot_path(
    tmp_path,
    monkeypatch,
):
    import env.drake_trajectory as drake_module

    real_evaluate = drake_module.evaluate_drake_foh
    calls = 0

    def counted_evaluate(action, tau):
        nonlocal calls
        calls += 1
        return real_evaluate(action, tau)

    monkeypatch.setattr(drake_module, "evaluate_drake_foh", counted_evaluate)
    train_sfps(
        _small_bank(seed=103),
        tmp_path,
        config=_short_b2_config(max_updates=2, validation_interval=2),
        root_seed=104,
        device="cpu",
    )

    # Four validation trajectories each contribute 58 windows. Training
    # targets are generated by the equivalent batched FOH path instead.
    assert calls == 4 * 58


def test_sfpd_training_keeps_samplewise_drake_out_of_the_training_hot_path(
    tmp_path,
    monkeypatch,
):
    import env.drake_trajectory as drake_module
    ...
    monkeypatch.setattr(drake_module, "evaluate_drake_foh", counted_evaluate)
    train_sfpd(
        _small_bank(seed=105),
        tmp_path,
        config=_short_config(max_updates=2, validation_interval=2),
        root_seed=106,
        device="cpu",
    )
