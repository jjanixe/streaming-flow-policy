...
root=Path(__file__).parent/'mode-aligned-fits'
...
for ax,(key,label,percentage) in zip(axes,fields):
 ...
 if percentage:ax.set_ylim(45,110);ax.set_yticks([50,60,70,80,90,100]);ax.set_ylabel('Percent')
 else:ax.set_ylim(0,.35);ax.set_ylabel('Mean absolute error')
fig.suptitle('Mode-indicator two-group fitting | 20 feedback seeds × 8 synthetic profiles',fontsize=13)
