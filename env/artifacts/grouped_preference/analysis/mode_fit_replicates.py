"""Mode-indicator synthetic-feedback repetitions; no policy rollouts or tuning."""
import json
...
out=Path(__file__).parent/'mode-aligned-fits'
out.mkdir(exist_ok=True)
rows=[]
users=synthetic_grouped_users()
held=build_grouped_queries(seed=190001,count=512)
hf=grouped_features(held['trajectories'],feature_kind='mode'); hd=hf[:,0]-hf[:,1]
for seed in range(20):
    query=build_grouped_queries(seed=180000+seed,count=40)
    f=grouped_features(query['trajectories'],feature_kind='mode'); delta=f[:,0]-f[:,1]
...
(out/'fit_replicates.json').write_text(json.dumps(dict(config=dict(feature_kind='mode',feedback_seeds=20,users=8,rho=16,l2=.1,heldout_pairs=512),summary=summary,rows=rows),indent=2))
